...
const auto inode1 = std::make_shared<InputNode>( input1 );
const auto inode2 = std::make_shared<InputNode>( input2 );
TEST_CASE( "test_node", "[test_node concat]" ) {
    ...
    const OperatorNode<decltype( ConcatOp )> op_node{ std::vector<std::shared_ptr<InputNode>>{ inode1, inode2 }, ConcatOp } ;
    REQUIRE( op_node.compute() == "Hello World " );
}

TEST_CASE( "test_node", "[test_node repeat5]" ) {
    const auto RepeatOpImpl = [] ( const std::string& input, int times, const std::string& delim  = "" ) {
        std::string res;
        if( times == 0 ) {
            return std::string();
        }
    ...
    const OperatorNode<decltype( RepeatOp5 )> op_node{ std::vector<std::shared_ptr<InputNode>>{ inode1 }, RepeatOp5 } ;
